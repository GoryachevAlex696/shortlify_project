class Api::V1::UsersController < Api::V1::BaseController
  ...
  # GET /api/v1/users/:id/followers
    def followers
    users = @user.followers.limit(20) # ← исправлено
    render json: {
        users: users,
        count: users.count
    }
    ...
    def destroy
        ...
        if @user == current_user
            # Дополнительная логика перед удалением (опционально)
            ...
            render json: { 
            status: :error, 
            error: 'Можно удалить только свой аккаунт' 
            }, status: :forbidden
        end
    end
